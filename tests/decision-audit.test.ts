...
test("policy audit deterministically replays a complete interview", async () => {
  ...
  assert.equal(audit.summary.replayedTurns, 6);
  assert.equal(audit.summary.matchingSelections, 6);
  assert.equal(audit.summary.adaptiveDecisions, 2);
...
function buildCompleteSession(): {
  ...
  for (let sequenceNumber = 1; sequenceNumber <= 6; sequenceNumber += 1) {
